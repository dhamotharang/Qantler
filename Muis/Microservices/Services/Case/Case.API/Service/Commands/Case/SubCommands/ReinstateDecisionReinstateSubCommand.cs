...
{
  public class ReinstateDecisionReinstateSubCommand : SubCommand
  {
    ...
    {
      var note = await _dbContext.Translation.GetTranslation(Locale.EN, "ReinstateDecisionNote");

      var activityID = await _dbContext.Activity.InsertActivity(new Activity
      {
        Action = await _dbContext.Translation.GetTranslation(Locale.EN, "ReinstateDecision"),
        User = _user,
