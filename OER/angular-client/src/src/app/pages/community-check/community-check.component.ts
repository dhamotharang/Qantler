...
@Component({
  selector: 'app-community-check',
  templateUrl: './community-check.component.html'
})
export class CommunityCheckComponent implements OnInit {
  categories: any;
  translations: any;
  category: any;
  allContent: any[];
  ...
  constructor(private profileService: ProfileService, private messageService: MessageService, private generalService: ResourceService,
              private QRCService: QrcService, private translate: TranslateService,
              private spinner: NgxSpinnerService, public encService: EncService, private router: Router, private route: ActivatedRoute) {
  }
  ...
  getContent() {
    if (this.category) {
      ...
      this.QRCService.CommunityGetContent(this.profileService.userId, this.pageNumber, this.pageSize, this.category).subscribe((res: any) => {
        if (res.hasSucceeded) {
  ...
  getCategories() {
    this.spinner.show(undefined, {color: this.profileService.themeColor});
    this.QRCService.GetCommunityCategories(this.profileService.userId).subscribe((res: any) => {
      if (res.hasSucceeded) {
        if (res.returnedObject.length > 0) {
  ...
  getApprovedContent() {
    this.spinner.show(undefined, {color: this.profileService.themeColor});
    this.QRCService.CommunityGetApprovedListByUser(this.profileService.userId, this.pageNumber, this.pageSize).subscribe((res: any) => {
      if (res.hasSucceeded) {
        this.requests = res.returnedObject;
        this.totalRows = this.requests[0].totalrows;
      } else {
  ...
  navigateToVerify(contentType, contentId) {
    this.router.navigate(['/verify-content/' + contentType + '/' + contentId + '/' + this.encService.set(0) + '/1/' + this.encService.set(this.category), {
      pn: this.pageNumber,
      ps: this.pageStart
    }]);
  }
