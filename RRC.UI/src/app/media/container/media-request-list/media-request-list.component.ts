...
import { MediaRequestListService } from './media-request-list.service';
import { DatePipe } from '@angular/common';
import { Router } from '@angular/router';
@Component({
  selector: 'app-media-request-list',
  templateUrl: './media-request-list.component.html',
  styleUrls: ['./media-request-list.component.scss']
})
//r3g4n3chhxjol5pdid3oy32mu7u4dbkv2xpckqckyoai3p4agdgq
export class MediaRequestListComponent implements OnInit {
...
public rows:Array<any> = [];
  ...
  constructor(private common: CommonService,public router:Router,private mediarequestlistservice: MediaRequestListService,public datePipe:DatePipe) {
  	 this.common.breadscrumChange('Media Request List','List Page','');
    this.common.topBanner(true,'Media Request List','+ CREATE REQUEST','media-request-list');
    //this.getUserList([]);
    this.getservice();
  ...
  ngOnInit() {
    ...
    let username = userdet.username;
      ...
      this.mediarequestlistservice.memoFilterList("memos/",this.page,this.maxSize,1,username,this.filter_data.status,source,destination,this.filter_data.private,this.filter_data.priority,from_date,to_date,this.filter_data.smartSearch)
      .subscribe((res: any)=>{
  ...
  public getservice(){
    ...
    // let res.collection =[];
    // let res.count ='0';
    this.mediarequestlistservice.mediaList("Media/Home/AllModulesPending",this.page,this.maxSize,1,username).subscribe((res: any)=>{
      this.data = res.collection;
      if(this.data){
        this.length = res.count;
        //this.maxSize = this.data.count
   ...
   public getComboList(){
    ...
    let letter_id = '0';
    let requestData =[];
    this.mediarequestlistservice.memoCombos("memo/",letter_id,user_id).subscribe((res: any)=>{
    ...
    this.mediarequestlistservice.userList('User/', requestData).subscribe((res: any)=>{
      this.userList = res;
      });
  }
