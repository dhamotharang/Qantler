ALTER PROCEDURE [dbo].[Get_VehiclePreview] --  [Get_VehiclePreview] 362
...
BEGIN
	...
	insert into @result
    select V.PlateNumber,V.PlateColor,V.VehicleMake,V.VehicleModel,(select IIF(@P_Language = 'EN', UP.EmployeeName, UP.AREmployeeName) from UserProfile as UP where UP.UserProfileId= (case when VR.DriverID is null then VR.TobeDrivenbyDriverID else VR.DriverID end)) 
	...
	,VR.VehicleID
	,YearofManufacture,(select IIF(@P_Language = 'EN', UP.EmployeeName, UP.AREmployeeName) from UserProfile as UP where Up.UserProfileId = (select  VH.ActionBy from VehicleHistory as VH where VH.Action = 'Release' and VH.VehiclReqID=@P_VehicleRequestID))
	,(select IIF(@P_Language = 'EN', UP.EmployeeName, UP.AREmployeeName) from UserProfile as UP where Up.UserProfileId = (select VH.ActionBy from VehicleHistory as VH where VH.Action = 'Return' and VH.VehiclReqID=@P_VehicleRequestID))
	...
	Update @Result set ReturnMeridiem = (select ArDisplayName from M_Lookups where DisplayName='AM') where ReturnTime like '%AM%'
	Update @Result set ReturnTime = REPLACE(ReturnTime,'AM','') where ReturnTime like '%AM%'
	
	Update @Result set ReturnMeridiem = (select ArDisplayName from M_Lookups where DisplayName='PM') where ReturnTime like '%PM%'
	Update @Result set ReturnTime = REPLACE(ReturnTime,'PM','') where ReturnTime like '%PM%'
