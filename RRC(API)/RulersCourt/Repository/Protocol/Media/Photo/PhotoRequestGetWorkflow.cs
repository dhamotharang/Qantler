...
namespace RulersCourt.Repository.Protocol.Media.Photo
{
    public class PhotoRequestGetWorkflow : CommonWorkflow
    {
        public WorkflowBO GetPhotoRequestWorkflow(PhotoRequestWorkflowModel responseModel, string conn)
        {
            WorkflowBO bo = new Workflow.WorkflowBO();
            bo.Service = "MediaPhotoRequest";
            ...
            switch (responseModel.Action)
            {
                ...
                case "Approve":
                    if (responseModel.AssigneeID is null)
                    {
                        ...
                        bo.To = toActor;
                        if (responseModel.CurrentStatus == 92 && responseModel.PreviousApproverID != responseModel.FromID)
                        {
