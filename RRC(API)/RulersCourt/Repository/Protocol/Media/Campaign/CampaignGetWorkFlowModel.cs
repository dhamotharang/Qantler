...
{
    public class CampaignGetWorkFlowModel : CommonWorkflow
    {
        public WorkflowBO GetDiwanIdentityWorkflow(CampaignResponseModel responseModel, string conn)
        {
            WorkflowBO bo = new Workflow.WorkflowBO();
            bo.Service = "MediaNewCampaignRequest";
            ...
            switch (responseModel.Action)
            {
                ...
                case "Approve":
                    if (responseModel.AssigneeID is null)
                    {
                        ...
                        bo.To = toActor;
                        if (responseModel.CurrentStatus == 86 && responseModel.PreviousApproverID != responseModel.FromID)
                        {
