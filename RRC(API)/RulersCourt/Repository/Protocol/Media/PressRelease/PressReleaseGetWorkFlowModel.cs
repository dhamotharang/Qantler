...
{
    public class PressReleaseGetWorkFlowModel : CommonWorkflow
    {
        public WorkflowBO GetDiwanIdentityWorkflow(PressReleaseResponseModel responseModel, string conn)
        {
            WorkflowBO bo = new Workflow.WorkflowBO();
            bo.Service = "MediaNewPressReleaseRequest";
            ...
            switch (responseModel.Action)
            {
                ...
                case "Approve":
                    if (responseModel.AssigneeID is null)
                    {
                        ...
                        bo.To = toActor;
                        if (responseModel.CurrentStatus == 80 && responseModel.PreviousApproverID != responseModel.FromID)
                        {
