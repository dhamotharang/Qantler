...
{
    public class DesignRequestGetWorkFlow : CommonWorkflow
    {
        public WorkflowBO GetDiwanIdentityWorkflow(DesignResponseModel responseModel, string conn)
        {
            WorkflowBO bo = new Workflow.WorkflowBO();
            bo.Service = "MediaDesignRequest";
            ...
            switch (responseModel.Action)
            {
                ...
                case "Approve":
                    if (responseModel.AssigneeID is null)
                    {
                        ...
                        bo.To = toActor;
                        if (responseModel.CurrentStatus == 68 && responseModel.PreviousApproverID != responseModel.FromID)
                        {
