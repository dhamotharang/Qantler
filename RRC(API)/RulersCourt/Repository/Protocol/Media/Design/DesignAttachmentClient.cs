...
{
    public class DesignAttachmentClient
    {
        public List<DesignAttachmentGetModel> GetDesignAttachmentById(string connString, int? serviceID, string type)
        {
            SqlParameter[] param = { new SqlParameter("@P_ServiceID", serviceID),
                                     new SqlParameter("@Type", type)
            };

            return SqlHelper.ExecuteProcedureReturnData<List<DesignAttachmentGetModel>>(connString, "Get_AttachmentByID", r => r.TranslateAsDesignAttachmentList(), param);
        }

        public string PostDesignAttachments(string connString, string type, List<DesignAttachmentGetModel> datas, int? serviceID)
        {
            ...
            foreach (DesignAttachmentGetModel data in datas)
            {
