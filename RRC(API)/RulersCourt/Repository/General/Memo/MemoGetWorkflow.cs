...
{
    public class MemoGetWorkflow : CommonWorkflow
    {
        public Workflow.WorkflowBO GetMemoWorkflow(MemoWorkflowModel responseModel, string conn)
        {
            ...
            switch (responseModel.Action)
            {
                ...
                case "Resubmit":
                    ...
                    SqlParameter[] param = {
                    new SqlParameter("@P_UserID", responseModel.ApproverID) };
                    deletgateUser = SqlHelper.ExecuteProcedureReturnString(conn, "Get_DelegateUser", param);
                    if (deletgateUser != null && deletgateUser != "0")
                        bo.DelegateTo = GetActor(Convert.ToInt32(deletgateUser), conn);
                    bo.WorkflowProcess = Workflow.WorkflowType.SubmissionWorkflow;
                    break;
                ...
                case "Approve":
                    ...
                    foreach (MemoDestinationUsersGetModel item in responseModel.DestinationUserID)
                        toActor.Add(GetActor(item.MemoDestinationUsersID, conn));

                    bo.To = toActor;
                    if (responseModel.CurrentStatus == 2 && responseModel.PreviousApproverID != responseModel.FromID)
                    {
                ...
                case "Redirect":
                    if (responseModel.CurrentStatus == 2)
                    {
                        bo.To = new System.Collections.Generic.List<Workflow.Actor>() { GetActor(responseModel.ApproverID, conn) };
                    ...
                    else if (responseModel.CurrentStatus == 3)
                    {
                ...
                case "Share":
                    ...
                    foreach (ShareparticipationUsersModel item in responseModel.ShredUserID)
                        toAct.Add(GetActor(item.UserID, conn));
                    bo.To = toAct;
                    bo.WorkflowProcess = Workflow.WorkflowType.ShareWorkflow;
                    break;
