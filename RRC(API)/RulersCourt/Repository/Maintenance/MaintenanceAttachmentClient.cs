...
{
    public class MaintenanceAttachmentClient
    {
        public List<MaintenanceAttachmentGetModel> GetAttachmentById(string connString, int? serviceID, string type)
        {
            SqlParameter[] param = { new SqlParameter("@P_ServiceID", serviceID),
                                     new SqlParameter("@Type", type)
            };
            return SqlHelper.ExecuteProcedureReturnData<List<MaintenanceAttachmentGetModel>>(connString, "Get_AttachmentByID", r => r.TranslateAsGetMaintenanceAttachmentList(), param);
        }

        public string MaintenancePostAttachments(string connString, string type, List<MaintenanceAttachmentGetModel> datas, int? serviceID)
        {
            ...
            foreach (MaintenanceAttachmentGetModel data in datas)
            {
