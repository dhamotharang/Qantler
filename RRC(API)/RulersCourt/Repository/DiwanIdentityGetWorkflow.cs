...
{
    public class DiwanIdentityGetWorkflow : CommonWorkflow
    {
        public Workflow.WorkflowBO GetDiwanIdentityWorkflow(DiwanIdentityWorkflowModel responseModel, string conn)
        {
            WorkflowBO bo = new Workflow.WorkflowBO();
            bo.Service = "DiwanIdentity";
            ...
            switch (responseModel.Action)
            {
                ...
                case "Approve":
                    if (responseModel.AssigneeID is null)
                    {
                        ...
                        bo.To = toActor;
                        if (responseModel.CurrentStatus == 100 && responseModel.PreviousApproverID != responseModel.FromID)
                        {
